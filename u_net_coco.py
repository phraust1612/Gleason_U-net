...
for i in range (len (imgIds)):
  ...
  I = scipy.misc.imresize(I, (512,512))
  DATA.append (I)
...
y=tf.placeholder(tf.float32,[None,512,512])
tf_drop = tf.placeholder(tf.float32)

L1_0 = tf.reshape(x,[-1,512,512,3])
...
L1_1 = tf.nn.conv2d(L1_0,W1_1,strides=[1,1,1,1],padding="SAME")
...
L2_0 = tf.nn.max_pool(L1_2,ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME")
L2_0 = tf.nn.dropout(L2_0,keep_prob=tf_drop)

# L2_0 : 256 x 256 x 64
# 3x3 conv + ReLU layer
W2_1 = tf.Variable(tf.random_normal([3,3,64,128],stddev=0.5),name="W2_1")
L2_1 = tf.nn.conv2d(L2_0,W2_1,strides=[1,1,1,1],padding="SAME")
...
# 2x2 max-pooling layer
L3_0 = tf.nn.max_pool(L2_2,ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME")
L3_0 = tf.nn.dropout(L3_0,keep_prob=tf_drop)

# L3_0 : 128 x 128 x 128
# 3x3 conv + ReLU layer
W3_1 = tf.Variable(tf.random_normal([3,3,128,256],stddev=0.5),name="W3_1")
L3_1 = tf.nn.conv2d(L3_0,W3_1,strides=[1,1,1,1],padding="SAME")
...
# 2x2 max-pooling layer
L4_0 = tf.nn.max_pool(L3_2,ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME")
L4_0 = tf.nn.dropout(L4_0,keep_prob=tf_drop)

# L4_0 : 64 x 64 x 256
# 3x3 conv + ReLU layer
W4_1 = tf.Variable(tf.random_normal([3,3,256,512],stddev=0.5),name="W4_1")
L4_1 = tf.nn.conv2d(L4_0,W4_1,strides=[1,1,1,1],padding="SAME")
...
# 2x2 max-pooling layer
L5_0 = tf.nn.max_pool(L4_2,ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME")
L5_0 = tf.nn.dropout(L5_0,keep_prob=tf_drop)

# L5_0 : 32 x 32 x 512
# 3x3 conv + ReLU layer
W5_1 = tf.Variable(tf.random_normal([3,3,512,1024],stddev=0.5),name="W5_1")
L5_1 = tf.nn.conv2d(L5_0,W5_1,strides=[1,1,1,1],padding="SAME")
...
L6_0 = tf.nn.conv2d_transpose(L5_2,W6_0,[batch_size,64,64,512],strides=[1,2,2,1],padding="SAME")
L6_0 = tf.concat([L4_2,L6_0],-1)

# L6_0 : 64 x 64 x 1024
# 3x3 conv + ReLU layer
W6_1 = tf.Variable(tf.random_normal([3,3,1024,512],stddev=0.5),name="W6_1")
L6_1 = tf.nn.conv2d(L6_0,W6_1,strides=[1,1,1,1],padding="SAME")
L6_1 = tf.nn.relu(L6_1)

# 3x3 conv + ReLU layer
W6_2 = tf.Variable(tf.random_normal([3,3,512,512],stddev=0.5),name="W6_2")
L6_2 = tf.nn.conv2d(L6_1,W6_2,strides=[1,1,1,1],padding="SAME")
L6_2 = tf.nn.relu(L6_2)

# 2x2 up-conv + concatenation
W7_0 = tf.Variable(tf.random_normal([2,2,256,512],stddev=0.5),name="W7_0")
L7_0 = tf.nn.conv2d_transpose(L6_2,W7_0,[batch_size,128,128,256],strides=[1,2,2,1],padding="SAME")
L7_0 = tf.concat([L3_2,L7_0],-1)

# L7_0 : 128 x 128 x 512
# 3x3 conv + ReLU layer
W7_1 = tf.Variable(tf.random_normal([3,3,512,256],stddev=0.5),name="W7_1")
L7_1 = tf.nn.conv2d(L7_0,W7_1,strides=[1,1,1,1],padding="SAME")
L7_1 = tf.nn.relu(L7_1)

# 3x3 conv + ReLU layer
W7_2 = tf.Variable(tf.random_normal([3,3,256,256],stddev=0.5),name="W7_2")
L7_2 = tf.nn.conv2d(L7_1,W7_2,strides=[1,1,1,1],padding="SAME")
L7_2 = tf.nn.relu(L7_2)

# 2x2 up-conv + concatenation
W8_0 = tf.Variable(tf.random_normal([2,2,128,256],stddev=0.5),name="W8_0")
L8_0 = tf.nn.conv2d_transpose(L7_2,W8_0,[batch_size,256,256,128],strides=[1,2,2,1],padding="SAME")
L8_0 = tf.concat([L2_2,L8_0],-1)

# L8_0 : 256 x 256 x 256
# 3x3 conv + ReLU layer
W8_1 = tf.Variable(tf.random_normal([3,3,256,128],stddev=0.5),name="W8_1")
L8_1 = tf.nn.conv2d(L8_0,W8_1,strides=[1,1,1,1],padding="SAME")
L8_1 = tf.nn.relu(L8_1)

# 3x3 conv + ReLU layer
W8_2 = tf.Variable(tf.random_normal([3,3,128,128],stddev=0.5),name="W8_2")
L8_2 = tf.nn.conv2d(L8_1,W8_2,strides=[1,1,1,1],padding="SAME")
L8_2 = tf.nn.relu(L8_2)

# 2x2 up-conv + concatenation
W9_0 = tf.Variable(tf.random_normal([2,2,64,128],stddev=0.5),name="W9_0")
L9_0 = tf.nn.conv2d_transpose(L8_2,W9_0,[batch_size,512,512,64],strides=[1,2,2,1],padding="SAME")
L9_0 = tf.concat([L1_2,L9_0],-1)

# L9_0 : 512 x 512 x 128
# 3x3 conv + ReLU layer
W9_1 = tf.Variable(tf.random_normal([3,3,128,64],stddev=0.5),name="W9_1")
L9_1 = tf.nn.conv2d(L9_0,W9_1,strides=[1,1,1,1],padding="SAME")
L9_1 = tf.nn.relu(L9_1)

# 3x3 conv + ReLU layer
W9_2 = tf.Variable(tf.random_normal([3,3,64,64],stddev=0.5),name="W9_2")
L9_2 = tf.nn.conv2d(L9_1,W9_2,strides=[1,1,1,1],padding="SAME")
...
# final layer L10 : 512 x 512 x 1

# L10 = tf.transpose(L10,perm=[0,2,1,3])
L10 = tf.reshape(L10, [-1,512,512])
